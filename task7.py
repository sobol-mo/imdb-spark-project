...
def task7(spark_session, write_in_file):
    ...
    most_popular_df = (
        ratings_df.withColumn('rank', f.dense_rank().over(window)).filter('rank<=10').orderBy(f.desc('decade')))
    ...
    most_popular_df = (
        most_popular_df.withColumn('max_votes', f.max('numVotes').over(window)).orderBy(f.desc('decade')))
    # most_popular_df.show(50)
