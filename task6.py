"""Task 6. Get information about how many episodes in each TV Series. Get the top
...
def task6(spark_session, write_in_file):
    ...
    title_episode_df = read_spark_df(spark_session,
                                     sts.TITLE_EPISODE_PATH, sts.title_episode_schema)

    title_episode_df = (title_episode_df
                        .withColumn('seasonNumber', f.when(f.col('seasonNumber') == '\\N', 0)
                                    .otherwise(f.col('seasonNumber')).cast('int')))
    title_episode_df = (title_episode_df
                        .withColumn('episodeNumber', f.when(f.col('episodeNumber') == '\\N', 0)
    ...
    series_grouped = (title_episode_df
                      .filter((f.col('seasonNumber') != 0) & (f.col('episodeNumber') != 0))
                      .groupBy('parentTconst').count()
                      .orderBy('count', ascending=False))
    series_grouped = series_grouped.limit(50)
    series_grouped.show()
    title_basics_df = (title_basics_df
                       .select(f.col('parentTconst'), f.col('primaryTitle'), f.col('titleType')))
